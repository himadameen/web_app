...
const Auth = () => {
    ...
    const submit = async (e) => {
        ...
        if (info.email !== "" && info.email !== "") {
            const url = 'http://localhost:2400/user/create';
            const resp = await axios.post(url, info)
            // localStorage.setItem('token', resp.data.token);
            localStorage.setItem('name', resp.data.response.fullName)
            if(resp.status === 200){
                navigate('/login')
            }else{
                                ...
                                <div className="col-lg-12 login-form">
                                    <form>
                                        ...
                                        <div className="col-lg-12 loginbttm">
                                            <Link to='/login' id='alr'><div id='alr_dy'>Already have a account</div></Link>
                                            <div className="col-lg-6 login-btm login-button">
                                                <button className="btn btn-outline-primary" onClick={submit}>SIGNUP</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
